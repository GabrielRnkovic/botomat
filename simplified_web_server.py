...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("simplified_web_server.log"), logging.StreamHandler()]
)
...
# Camera streams
camera_feeds = {
    1: None,  # Will be set to built-in webcam
    2: None   # Will be set to test video
}
...
def initialize_system():
    ...
    with init_lock:
        logger.info("Initializing simplified speed camera system...")
        ...
        print("\n==================================================")
        print("SIMPLIFIED SPEED CAMERA SYSTEM")
        print("==================================================")
        print("- Camera 1: Built-in webcam")
        print("- Camera 2: Test video (since Camo is not available)")
        print("- Distance setting:", speed_calculator.distance_meters, "meters")
        print("==================================================\n")
        
        # Set up first camera - built-in webcam
        ...
        # Set up second camera - use test video
...
def process_camera_feed(camera_id):
    ...
    detection_interval = 3  # seconds between forced detections for testing
    ...
    while system_status["running"]:
        ...
        # Also force processing periodically for testing
        force_detection = (current_time - last_detection_time) > detection_interval
        
        if process_this_frame or force_detection:
            ...
            if plate and confidence > 0.3:  # Lowered threshold for more detections
                ...
                # If no plate detected with high confidence but we need to force a detection,
                # create a simulated detection
                if force_detection:
                    # Generate a random plate for testing
                    test_plates = ["ABC123", "XYZ789", "DEF456", "GHI789"]
                    sim_plate = test_plates[int(current_time) % len(test_plates)]
                    
                    # Save frame anyway
                    timestamp = int(current_time)
                    image_filename = f"detections/camera{camera_id}_{timestamp}_{sim_plate}.jpg"
                    cv2.imwrite(image_filename, frame)
                    
                    logger.info(f"Camera {camera_id} simulated plate detection: {sim_plate}")
                    
                    # Process the simulated detection
                    measurement = speed_calculator.process_detection(
                        camera_id=camera_id,
                ...
                logger.debug(f"No real plate detected in frame {frame_count} from camera {camera_id} or confidence too low")
